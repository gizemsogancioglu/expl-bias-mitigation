...
import pandas as pd
from source.data_loader import data, get_features, get_gender
from source.proxyMute import get_expl, ours_, iterative_analysis_
from source.training import blackbox_classifier
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        get_expl("val_"+str(i), classifier, profiles, type='clf')

        # STEP 3: iteratively disable each feature cumulatively (ranked by importance decreasingly) and save results.
        iterative_analysis_(f"val_{i}", profiles, biased_labels, blind_labels, f"blind-test_{i}")
        iterative_analysis_(f"val_{i}", profiles, biased_labels, biased_labels, f"biased-test_{i}")
